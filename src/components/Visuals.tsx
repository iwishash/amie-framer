...
const VisualFrame = ({ children, id }: VisualFrameProps) => {
  return (
    <div
      className={cn(
        "fixed inset-0 mx-auto flex max-w-6xl items-center justify-center px-4 opacity-0",
        `fullScreen-${id}`,
      )}
    >
      {children}
...
export const MusicVisual = ({ id }: VisualProps) => {
...
export const OtherVisual = ({ id }: VisualProps) => {
  return (
    <VisualFrame id={id}>
      <img src="/window-spotify.webp" />
    </VisualFrame>
